...
if(isset($_POST['envoi_message'])) {
  ...
  $message = htmlspecialchars($_POST['message']);
         ...
         $ins = $bdd->prepare('INSERT INTO msg1(champname, message) VALUES (?,?)'); 
...
if ((in_array($extensionFichier, $extensionsAutorisees))) {
    ...
    if (move_uploaded_file($_FILES["monfichier"]["tmp_name"],$repertoireDestination.$nomDestination)) {
        $insertimg = $bdd->prepare("UPDATE msg1 SET image = ? WHERE message = ?");
...
<div class="file">
 ...
 <input type="submit" value="Send" class="submit" name="envoi_message"/>
  </form>
</div>
<script>
setInterval('load_messages1()', 3000);
function load_messages1() {
  $('#messages').load('load_messages1.php');
}
</script>
